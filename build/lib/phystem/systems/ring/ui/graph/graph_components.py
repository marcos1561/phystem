...
from .graphs_cfg import ParticleCircleCfg, RegionsCfg, CellAreaCfg
...
class Density(ColorBarableComp):
    def __init__(self, ax: Axes, active_rings: ActiveRings, cell_shape: tuple, sim_configs, artist_kwargs: dict, colorbar_kwargs=None):
        super().__init__(ax, show_cfg_name="show_density")
        if colorbar_kwargs is None:
            colorbar_kwargs = {}
        if "label" not in colorbar_kwargs.keys():
            colorbar_kwargs["label"] = r"$\rho/\rho_{eq} - 1$"

        
        if "vmax" not in artist_kwargs.keys():
            artist_kwargs["vmax"] = 0.5
        if "vmin" not in artist_kwargs.keys():
            artist_kwargs["vmin"] = -0.5
        ...
        dynamic_cfg: RingCfg = sim_configs["dynamic_cfg"]

        l, h = space_cfg.length, space_cfg.height
        ring_d = dynamic_cfg.get_ring_radius() * 2
        ...
        self.density_eq = 1 / dynamic_cfg.get_equilibrium_area()
        self.density_calc = rings_quantities.Density(grids.RegularGrid(l, h, num_cols, num_rows))
    ...
    def get_density(self):
        ...
        # return count
        return self.density_calc.get_from_cm(self.active_rings.cms)/self.density_eq - 1

    def update_artists(self):
        self.artist.set_array(self.get_density())

class CellArea(ColorBarableComp):
    def __init__(self, ax: Axes, active_rings: ActiveRings, configs: CellAreaCfg, sim_configs: dict):
        super().__init__(ax, show_cfg_name="show_cell_area")
        ...
        if self.configs.area0 is None:
            self.configs.area0 = dynamic_cfg.area0 

        l, h = space_cfg.length, space_cfg.height
        ring_d = dynamic_cfg.get_ring_radius() * 2
        num_rows = int(h/(ring_d * self.configs.cell_shape[0]))
        num_cols = int(l/(ring_d * self.configs.cell_shape[1]))
        ...
        self.grid = grids.RegularGrid(
            length=l, height=h,
            num_cols=num_cols, num_rows=num_rows,
        )

        self.artist = ax.pcolormesh(*self.grid.edges, self.get_area(), shading='flat',
            zorder=1, cmap="coolwarm", **self.configs.artist_kwargs)
        self.artist.remove()

        self.artist_list.add("main", self.artist)

    def get_color_bar(self):
        return self.ax.figure.colorbar(self.artist, ax=self.ax, **self.configs.colorbar_kwargs)

    def get_area(self):
        areas = np.array(self.active_rings.solver.area_debug.area)[self.active_rings.ids]
        coords = self.grid.coords(self.active_rings.cms, simplify_shape=True)
        return self.grid.mean_by_cell(areas, coords, simplify_shape=True, remove_out_of_bound=True) / self.configs.area0

    def update_artists(self):
        self.artist.set_array(self.get_area())
