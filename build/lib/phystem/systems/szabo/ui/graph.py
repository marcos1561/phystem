...
class ParticlesGraph:
    ...
    def init(self):
        ...
        for x_i, y_i in zip(self.pos[0], self.pos[1]):
            self.circles.append(Circle((x_i, y_i), self.dynamic_cfg.r_eq/2, fill=False))
...
class Info(graph.Info):
    def __init__(self, ax: Axes, solver: CppSolver, time_it: TimeIt, 
        self_propelling_cfg: SelfPropellingCfg, creator_cfg: CreatorCfg, space_cfg: SpaceCfg) -> None:
        super().__init__(ax)
        self.solver = solver
