...
class CollectDataCfg(RunCfg):
    ...
    def __init__(self, int_cfg: IntegrationCfg, tf: float, folder_path: str, func_cfg=None, func: callable=None,
        ...
        * Passando o id de uma pipeline (`func_id`) e uma função que retorna a pipeline por id (`get_func`).  
        ...
        if self.is_autosave:
            correct_folder_path = self.checkpoint.root_path.parents[1]
            if correct_folder_path != self.folder_path:
                self.folder_path = correct_folder_path
...
class RealTimeCfg(RunCfg):
    ...
    def __init__(self, int_cfg: IntegrationCfg, num_steps_frame: int, fps: int=30, graph_cfg=None,
        ui_settings: config_ui.UiSettings=None, checkpoint: CheckpointCfg=None, ignore_app=None, fig_ax=None) -> None:
        '''
        Parameters
        -----------
            int_cfg:
        ...
        * tf
        ...
        if ui_settings is None:
            if replay is not None:
                ui_settings = replay.ui_settings
            else:
                ui_settings = config_ui.UiSettings()
