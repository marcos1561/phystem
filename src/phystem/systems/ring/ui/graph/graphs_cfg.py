...
class CellAreaCfg:
    def __init__(self, cell_shape=None, area0=None, artist_kwargs=None, colorbar_kwargs=None):
        ...
        if "vmax" not in artist_kwargs.keys():
            artist_kwargs["vmax"] = 1.1
        if "vmin" not in artist_kwargs.keys():
            artist_kwargs["vmin"] = 0.9
        
        if colorbar_kwargs is None:
            colorbar_kwargs = {}
        if "label" not in colorbar_kwargs:
            colorbar_kwargs["label"] = "A/A_0"

        self.cell_shape = cell_shape
        self.area0 = area0
        self.artist_kwargs = artist_kwargs
...
class SimpleGraphCfg(BaseGraphCfg):
    def __init__(self, begin_paused=False, pause_on_high_vel=False, show_scatter=False, show_density=False,
        ...
        show_particle_info=False, obstacle_cfg: ObstacleCfg=None, 
