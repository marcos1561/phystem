...
class DeltaColTime(collectors.RingCol):
    def __init__(self, wait_time, xlims, start_dt, check_dt,
        ...
        Parâmetros:
            wait_time:
                Tempo esperado para finalizar uma medida.
        ...
        self.ring_diameter = utils.ring_radius(
    ...
    def collect(self):
        if self.state is State.waiting:
            # TODO: Se em uma passo temporal é maior do que self.start_dt, 
            # então o self.check() nunca será executado.
            if self.solver.time > (self.last_start_time + self.start_dt):
                self.state = State.starting
            else:
                if self.solver.time > self.last_check_time + self.check_dt:
                    self.last_check_time = self.solver.time
    ...
    def start(self):
        ...
        selected_ids = ids_region[in_center_mask]
        selected_uids = uids_region[in_center_mask]

        self.tracking.add(
            id=selected_ids, 
            uid=selected_uids,
            end_x=self.solver.time + self.wait_time,
            dp_id=self.data_point_id,
            ids_region=ids_region,
        )
    ...
    def check(self):
        ...
        has_calc_active = False
        for idx in range(self.tracking.size):
            t_id, end_time = self.tracking.get("ids", idx), self.tracking.get("end_x", idx)
            
            if self.solver.time < end_time:
                continue
